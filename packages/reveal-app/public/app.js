...
exportBtn.addEventListener('click', async () => {
    ...
    const payload = { cardIndex: selectedIndex, format };
    showStatus(`Exporting card ${selectedIndex} as ${format}...`);

    try {
        showStatus(`Sending POST /export ...`);
        const res = await fetch('/export', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload),
        });
        showStatus(`Response: ${res.status} ${res.statusText}`);
        const text = await res.text();
        showStatus(res.ok ? `Exported: ${JSON.parse(text).path}` : `Export failed (${res.status}): ${text}`);
    } catch (err) {
        showStatus(`Export fetch error: ${err.message}`);
    }
