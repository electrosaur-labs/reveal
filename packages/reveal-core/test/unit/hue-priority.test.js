...
describe('Hue-Aware Priority Multiplier (v0.2.0)', () => {
    describe('_getHueSector()', () => {
        test('should map red (a=50, b=0) to sector 0', () => {
            const sector = PosterizationEngine._getHueSector(50, 0);
            expect(sector).toBe(0); // Red: 0-30°
        });

        test('should map orange (a=50, b=25) to sector 0 or 1', () => {
            const sector = PosterizationEngine._getHueSector(50, 25);
        ...
        test('should map cyan (a=-50, b=-10) to sector 5 or 6', () => {
            const sector = PosterizationEngine._getHueSector(-50, -10);
        ...
        test('should map purple (a=20, b=-50) to sector 8 or 9', () => {
            const sector = PosterizationEngine._getHueSector(20, -50);
            expect(sector).toBeGreaterThanOrEqual(8);
            expect(sector).toBeLessThanOrEqual(9);
        });

        test('should return -1 for grayscale (low chroma)', () => {
            expect(PosterizationEngine._getHueSector(2, 1)).toBe(-1);
            expect(PosterizationEngine._getHueSector(0, 0)).toBe(-1);
            expect(PosterizationEngine._getHueSector(3, 3)).toBe(-1);
        });
    ...
    describe('_calculateBoxMetadata()', () => {
        ...
        test('should ignore chroma in grayscaleOnly mode', () => {
            ...
            const colorMetadata = PosterizationEngine._calculateBoxMetadata(box, false);
            const grayMetadata = PosterizationEngine._calculateBoxMetadata(box, true);
    ...
    describe('_calculateSplitPriority()', () => {
        test('should return base variance when sector is covered', () => {
            ...
            const priority = PosterizationEngine._calculateSplitPriority(
        ...
        test('should ignore hue priority in grayscale mode', () => {
            ...
            const metadata = PosterizationEngine._calculateBoxMetadata(box, true);
        ...
        test('should use custom hueMultiplier parameter', () => {
            ...
            const priority2x = PosterizationEngine._calculateSplitPriority(
