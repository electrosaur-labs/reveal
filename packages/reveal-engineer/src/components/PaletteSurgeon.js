    ...
    _rebuild() {
        ...
        for (let i = 0; i < rgbPalette.length; i++) {
            ...
            swatch.addEventListener('pointerdown', (e) => {
                ...
                // Capture idx and shift/alt keys from THIS pointerdown — do not
                // read this._dragSourceIndex at fire time, which can be stale if
                // another swatch receives a pointerdown before this pointerup fires.
                const capturedIdx = idx;
                const capturedShift = e.shiftKey;
                const capturedAlt = e.altKey;
                const onUp = (ev) => {
                    ...
                    if (wasDragging && targetIdx >= 0 && targetIdx !== capturedIdx &&
                        !this._session.deletedColors.has(targetIdx)) {
                        logger.log(`[Surgeon] MERGE ${capturedIdx}→${targetIdx} (drag)`);
                        this._session.mergePaletteColors(capturedIdx, targetIdx).catch(err => {
                            logger.log(`[PaletteSurgeon] Merge failed: ${err.message}`);
                        });
                    } else if (!wasDragging) {
                        this._onSwatchClick(capturedIdx, capturedShift, capturedAlt);
                    }
                };
